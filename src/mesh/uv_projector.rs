...
mod tests {
    ...
    mod projection_tests {
        ...
        fn test_box_projector_simple_triangle() {
            let vertices = vec![
                Vertex { position: [0.0, 0.0, 0.0], normal: [0.0, 0.0, 1.0], tex_coords: [0.0, 0.0] },
                Vertex { position: [1.0, 0.0, 0.0], normal: [0.0, 0.0, 1.0], tex_coords: [0.0, 0.0] },
                Vertex { position: [0.0, 1.0, 0.0], normal: [0.0, 0.0, 1.0], tex_coords: [0.0, 0.0] },
            ];
            ...
            for v in new_vertices {
                assert!(v.tex_coords[0] >= 2.0, "U coord {} should be in tile 2", v.tex_coords[0]);
                assert!(v.tex_coords[0] <= 2.5, "U coord {} should be in tile 2", v.tex_coords[0]);
                assert!(v.tex_coords[1] >= 0.0, "V coord {} should be positive", v.tex_coords[1]);
                assert!(v.tex_coords[1] <= 1.0, "V coord {} should be less than 1.0", v.tex_coords[1]);
            }
        }
    }
    ...
    mod margin_tests {
        ...
        fn test_different_margins() {
            ...
            for (margin_size, margin_val) in margins {
                ...
                let min_u = new_vertices.iter().map(|v| v.tex_coords[0]).fold(f32::MAX, f32::min);
                let min_v = new_vertices.iter().map(|v| v.tex_coords[1]).fold(f32::MAX, f32::min);
                assert!((min_u - (2.0 + margin_val)).abs() < 1e-4, "Min U {} should be close to {}", min_u, 2.0 + margin_val);
                assert!((min_v - margin_val).abs() < 1e-4, "Min V {} should be close to {}", min_v, margin_val);
            }
        }
    }
}
